...
*all the F ratios use the *same* denominator* - [Douglas Bates](https://stat.ethz.ch/pipermail/r-help/2006-May/094765.html)
...
Se **recomienda** tener 10 veces más datos que parámetros.
...
De mejor a peor lo que podemos hacer es usar para comparar modelos son:
...
Otra cosa que podemos hacer es pedirle a R que construya los modelos con base a la información que le damo.  
```{r}
bmi_mod_comp <- lm(bmi ~., data = bmi_datos)
```
**Nota** tu dataframe solo debe incluir las variables que te interesa incluir.


Otro paquete que permite comparar modelos es **MASS**. 
```{r, warning=FALSE, message=FALSE}
library(MASS)
```

La ventaja de stepAIC es que te permite elegir: direction = c("both", "backward", "forward").
```{r}
